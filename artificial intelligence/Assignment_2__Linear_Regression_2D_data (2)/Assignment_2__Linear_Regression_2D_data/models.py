...
class linear_model:
    ...
    def backward(self, y_actl, y_pred, feat):
        ...
        '''
        n=len(y_actl)
        diff_arr=np.subtract(y_pred,y_actl)
        self.W=self.W-1/n*(self.lr)*np.dot(feat.transpose(),diff_arr)                                                                 
    ...
    for x in X:
        if choice == 'linear':
            ...
            for i in range(len(alpha)):
                listelem.append(alpha[i])
                listelem.append(1)
            feat.append(listelem)
            ...
            for i in range(len(alpha)):
                for j in range(param):
                    listelem.append(math.sin((j+1)*alpha[i]))
                    listelem.append(math.cos((j+1)*alpha[i]))
                listelem.append(1)
            feat.append(listelem)
