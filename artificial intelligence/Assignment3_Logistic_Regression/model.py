...
class LogisticRegression(object):
    ...
    def loss(self, X_batch, Y_batch):
        ...
        diffarray=prediction-Y_batch
        n=Y_batch.size
        grad = (1/n)*(np.dot(X_batch.T,diffarray))

        # calculate loss (stored in variable name loss) and gradients (stored in variable name grad)
        loss=(1/n)*(-np.dot(Y_batch,np.log(prediction))-np.dot((1-Y_batch),np.log(1-prediction)))
    ...
    def train(self,
        ...
        if self.add_bias:
            self.weights = np.random.randn(X_train.shape[1]+1) # Fill a correct value here
        else:
            self.weights = np.random.randn(X_train.shape[1]) # Fill a correct value here

        X_train_adj = None
        feat=X_train.shape[0]
        biasarray=np.ones(feat)

        # Create a new input array by adding bias column with all values one in X_train_adj array
        # while taking in consideration the bias (so two cases according to the boolean value of add_bias).
        if self.add_bias:
            X_train_adj = np.column_stack((X_train,biasarray))  # Fill a correct value here
        else:
            X_train_adj = X_train
        ...
        for i in range(num_iters):
            ...
            loss  = loss + (reg_const/(2*X_train_adj.shape[1]))*np.dot(self.weights.T,self.weights)
            grad=grad + (reg_const/X_train_adj.shape[1])*self.weights
